...
class CommandsModel(db.Model):
    ...
    search_url = db.Column(EncryptedType(1500))
    permission_level = db.Column(db.Integer, unique=False, nullable=False, default=999)
    is_command_public = db.Column(db.Boolean, unique=False, nullable=False)
    is_command_household = db.Column(db.Boolean, unique=False, nullable=False)
    ...
    @classmethod
    def create_command(cls, category, prefix, url, search_url, permission_level = 999, is_command_public = True, is_command_household = False, is_command_hidden = False, household_id = None, owner_id = None):
        new_command = cls(
            category=category,
            prefix=prefix,
            url=url,
